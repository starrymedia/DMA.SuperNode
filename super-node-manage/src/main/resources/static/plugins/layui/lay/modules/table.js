...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'fsConfig'], function (exports) {
        ...
        , thisTable = function () {
            ...
            return {
                ...
                query: function (params)//查询操作
                {
                    for (var k in params) {
                        var v = params[k];
                        if (v == null || v == 'undefined' || v == undefined || $.trim(v) == '') {
                            delete  params[k];
                        } else {
                            params[k] = $.trim(v);
                        }
        ...
        if (options.url && isLoad == "1") { //Ajax请求
            ...
            $.ajax({
                ...
                , error: function (e, m) {
                    ...
                    that.layMain.find('tbody').html('');
                    that.layMain.find('.' + NONE).remove();
                    that.layMain.append('<div class="' + NONE + '">Error</div>');
                    loadIndex && layer.close(loadIndex);
                }
            });
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        ...
        if (options.page) {
            options.page = $.extend({
                ...
                , count: count
                , limit: options.limit
                , limits: options.limits || [15, 20, 40, 60, 80, 100, 200, 500]
                , groups: 3
    ...
    Class.prototype.events = function () {
        var that = this
            ...
            , sort = this.layHeader.find('.layui-edge')
            , resizing
            , ELEM_CELL = '.layui-table-cell'
            , filter = options.elem.attr('lay-filter');

        function translate() {
            var item = window.localStorage.getItem(_lang_dic_key);
            if (item) {
                var wordsMap = JSON.parse(item);
                window.Translator.init(wordsMap).translate('span[data-sw-translate]');
            }
        }
        ...
        that.layBody.on('click', '*[lay-event]', function () {
            var othis = $(this);
            var jQuery = othis.parents('tr').eq(0);
            var index = jQuery.data('index')
...
//          table.cache[that.key][index] = [];
                  var delTotal=   table.cache[that.key].splice(index, 1);
                  if(!delTotal){
                      layer.msg({icon: 6});
                  }
                    tr.remove();
